  ...
  return (
    <AnimatePresence>
      {show && (
        <>
          {/* 亮度控制按鈕 - 右上角 */}
          {brightnessControl?.showIcon && (
            <motion.button
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.8 }}
              transition={{ duration: 0.2 }}
              className="floating-control top-4 right-16"
              onClick={(e) => {
                e.stopPropagation();
                brightnessControl.toggleBrightnessMode();
              }}
              aria-label={
                brightnessControl.brightnessMode === 'auto' && brightnessControl.isNightTime
                  ? '切換到日間模式'
                  : brightnessControl.brightnessMode === 'manual-night'
                  ? '切換到自動模式'
                  : '切換到夜間模式'
              }
            >
          ...
          {media && media.fileType !== 'image' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 50 }}
              transition={{ duration: 0.2 }}
              className={`fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-0 right-0 w-max mx-auto z-50 bg-black/70 backdrop-blur-sm rounded-2xl p-4 ${className}`}
            >
              {/* 進度條 */}
              {duration > 0 && (
          ...
          {media && media.fileType === 'image' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 50 }}
              transition={{ duration: 0.2 }}
              className="fixed bottom-[calc(1rem+env(safe-area-inset-bottom))] left-0 right-0 w-max mx-auto z-50 bg-black/70 backdrop-blur-sm rounded-2xl p-4"
            >
